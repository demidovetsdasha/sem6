...
# Дата: 10.05.2025
# Источники: В 2 ч. Ч.1: Формальные модели обработки информации и параллельные модели решения задач:
...
def find_C(x, y, m):
    ...
    def find_impl(a, b): # sup({ z| ((1-x)*z<=y) /\ (z<=1) })
        denominator = diff(1, a) 
        if abs(denominator) < 1e-5:  # Если знаменатель слишком мал 
            denominator = 1e-5 if denominator > 0 else -1e-5 
        return compare(div(b, denominator), 1, 0)
