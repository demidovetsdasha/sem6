...
Выполнил: студент гр.221701  Крупский А. В.
Вариант 9: (~) = /1\\; /~\\ = /1\\; x~>y = max({1-x}\\/{y})
...
def find_C(x, y, m):
    ...
    def find_kf(i, j):
        ...
        if 6 <= n <= m * 3:
            new_n = n - n % 3  # будет задействоваться максимальное n кратно 3
            count = math.ceil((m * 3) / new_n)  # сколько должно выполниться последоватеьных операций
    ...
    def find_kd(i, j):
        ...
        if 2 <= n <= m * 1:
            new_n = n - n % 1  # будет задействоваться элементов кратное 1
