...
import {
  ...
  simpleSeedFill,
  scanlineSeedFill
} from "../../logic/lab5/filling";

function NewPolygonEditor() {
  ...
  const [fillAlgorithm, setFillAlgorithm] = useState("ordered-edge-list");
  const [debugMode, setDebugMode] = useState(false);
  const [fillSteps, setFillSteps] = useState([]);
  const [currentStep, setCurrentStep] = useState(0);
  const [isFilling, setIsFilling] = useState(false);
  const plotRef = useRef(null);

  useEffect(() => {
    if (debugMode && fillSteps.length > 0) {
      const timer = setInterval(() => {
        setCurrentStep(prev => (prev < fillSteps.length - 1 ? prev + 1 : prev));
      }, 100);
  ...
  const handleFillPolygon = async () => {
    if (hullPoints.length < 3) {
      setStatus("Сначала постройте полигон!");
      return;
    }
    setIsFilling(true);
    setStatus("Начало заполнения...");
    let seed = hullPoints[0];
    if (testPoint && pointInPolygon(testPoint, hullPoints)) seed = testPoint;
    
    const algorithmMap = {
      "ordered-edge-list": () => orderedEdgeListFill(hullPoints),
      "active-edge-list": () => activeEdgeListFill(hullPoints),
      "simple-seed": () => simpleSeedFill(hullPoints, seed),
      "scanline-seed": () => scanlineSeedFill(hullPoints, seed)
    };

    try {
      const { filledPixels, steps } = algorithmMap[fillAlgorithm]();
      if (debugMode) {
        setFillSteps(steps);
        setCurrentStep(0);
      } else {
        setFillSteps([filledPixels]);
        setCurrentStep(0);
      }
      setStatus(`Заполнено ${filledPixels.length} пикселей`);
    } catch (error) {
  ...
  const generatePlotData = () => {
    ...
    return [
      ...
      ...(fillSteps.length > 0 && currentStep < fillSteps.length ? [{
        x: fillSteps[currentStep].map(p => p.x),
        y: fillSteps[currentStep].map(p => p.y),
        mode: "markers",
        type: "scatter",
        name: "Заливка",
        marker: { size: 4, color: debugMode ? "#FFC107" : "#4CAF50", opacity: debugMode ? 0.6 : 1 }
      }] : [])
    ];
  };
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      <div style={{ marginTop: 20, display: "flex", flexWrap: "wrap", gap: 10 }}>
        ...
        <select
          value={fillAlgorithm}
          onChange={e => setFillAlgorithm(e.target.value)}
          style={{ padding: "8px 16px", borderRadius: 4 }}
          disabled={isFilling}
        >
          <option value="ordered-edge-list">Упорядоченные рёбра</option>
          <option value="active-edge-list">Активные рёбра</option>
          <option value="simple-seed">Простая затравка</option>
          <option value="scanline-seed">Построчная затравка</option>
        </select>

        <button
          onClick={() => setDebugMode(!debugMode)}
          style={{ background: debugMode ? "#673AB7" : "#9E9E9E", color: "white", padding: "8px 16px" }}
        >
          {debugMode ? "Обычный режим" : "Режим отладки"}
        </button>

        <button
          onClick={handleFillPolygon}
          style={{ background: "#009688", color: "white", padding: "8px 16px" }}
          disabled={isFilling}
        >
          {isFilling ? "Заполнение..." : "Заполнить"}
        </button>
...
export default NewPolygonEditor;